...
from orbis_user_api.services.exceptions import NoteNotFound, NoteVersionConflict, NotebookNotFound, UserWorkspaceMissing
...
async def list_documents(
    ...
    try:
        return DocumentListResponse(items=await list_documents_service(user, session, notebook_id))
...
async def create_document(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Notebook not found") from None
...
async def update_document_content(
    ...
    try:
        return await update_document_content_service(document_id, payload, user, session)
    except NoteNotFound:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found") from None
    except NoteVersionConflict:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Document version conflict") from None
    except UserWorkspaceMissing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="User workspace is missing") from None
