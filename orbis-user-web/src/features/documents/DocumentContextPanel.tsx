...
export function DocumentContextPanel({ notebookId, activeNoteId, mobile = false, onNavigate }: DocumentContextPanelProps) {
  ...
  return (
    <section className="document-context" aria-label="文档目录">
      ...
      {!treeQuery.isLoading && !treeQuery.isError && !treeItems.length ? <div className="px-3 py-5 text-xs leading-5 text-[var(--muted)]">这个笔记本还没有文档。{canEdit ? "从第一篇文档开始吧。" : ""}</div> : null}
      {!treeQuery.isLoading && !treeQuery.isError ? <nav className="document-context-tree" aria-label="笔记本文档"><DocumentTree items={treeItems} depth={0} activeNoteId={activeNoteId} canEdit={canEdit} onNavigate={mobile ? onNavigate : undefined} onCreateChild={setCreateParent} onRename={setRenameTarget} onArchive={(note) => void handleArchive(note)} onMove={(note, delta) => void handleMove(note, delta)} /></nav> : null}
